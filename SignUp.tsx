...
const SignUp: React.FC<SignUpProps> = ({ onComplete }) => {
  const [form, setForm] = useState({ username: 'atlaskazuma', email: 'atlaskazuma@example.com', password: 'atlasthegreat' });
  ...
  return (
    <div className="max-w-md mx-auto bg-zinc-900/50 border border-zinc-800 p-8 rounded-lg shadow-2xl">
      <div className="mb-8 text-center">
        <h1 className="text-3xl font-bold tracking-tighter text-white">PATTERWATCH</h1>
        <p className="text-zinc-500 text-sm mt-1">Relationship Pattern Intelligence System</p>
      </div>
      ...
      <p className="mt-6 text-center text-zinc-500 text-xs">
        Already have an account? <span className="text-zinc-300 hover:text-white cursor-pointer underline">Login</span>
      </p>
    </div>
  );
};
