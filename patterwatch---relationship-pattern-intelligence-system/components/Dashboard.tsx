...
import { User, Partner, PatternAlert, Conversation } from "../types/types";
import { Terminal } from "./Terminal";
import {
  generateTerminalLogs,
...
const Dashboard: React.FC<DashboardProps> = ({ user, partner, onLogout }) => {
  ...
  const [activeTab, setActiveTab] = useState<"DASHBOARD" | "MCE" | "PATTERNS">(
    "DASHBOARD",
  );
  ...
  const toggleSystem = () => {
    ...
    setLogs((prev) => [
      ...prev,
      isSystemActive ? "SYSTEM_IDLE" : "SYSTEM_INITIALIZED: 100% — COMPLETED",
      "ALL_SYSTEMS_GO // ENCRYPTION_LAYER_01_ACTIVE",
    ]);
  };
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-zinc-900/40 border border-zinc-800 rounded-lg p-12 flex flex-col items-center justify-center relative overflow-hidden min-h-[400px]">
              {/* Background pulses */}
              <div
                className={`absolute w-[300px] h-[300px] rounded-full border border-green-500/10 ${isSystemActive ? "animate-ping" : ""}`}
              ></div>
              <div
                className={`absolute w-[200px] h-[200px] rounded-full border border-green-500/20 ${isSystemActive ? "animate-pulse" : ""}`}
              ></div>

              <div
                className={`relative w-48 h-48 rounded-full bg-black border-4 flex items-center justify-center flex-col transition-all duration-700 ${isSystemActive ? "border-green-500 shadow-[0_0_50px_rgba(0,255,65,0.2)]" : "border-zinc-800"}`}
              >
                {isSystemActive ? (
                  <>
                    <div className="text-green-500 animate-pulse font-mono text-xs tracking-widest mb-2">
                      ACTIVE
                    </div>
                    <div className="text-white font-bold text-3xl font-mono tracking-tighter">
                      100%
                    </div>
                    <div className="text-[10px] text-zinc-500 mt-2 font-mono uppercase tracking-[0.2em]">
                      Trust Core
                    </div>
                  </>
                ) : (
                  <>
                    <div className="text-zinc-700 font-mono text-xs tracking-widest mb-2">
                      IDLE
                    </div>
                    <div className="text-zinc-800 font-bold text-3xl font-mono tracking-tighter">
                      --%
                    </div>
                  </>
                )}
              ...
              <div className="mt-12 text-center">
                <p
                  className={`font-mono text-xs tracking-[0.3em] uppercase transition-colors ${isSystemActive ? "text-green-400" : "text-zinc-600"}`}
                >
            ...
            <div className="bg-red-950/20 border border-red-900/30 p-4 rounded text-center">
              <p className="text-[10px] text-red-500 font-mono uppercase tracking-widest">
                Abuse Safeguard: Any pattern manipulation will result in
      ...
      {activeTab === "PATTERNS" && (
        <PatternEngine isSystemActive={isSystemActive} />
      )}
    </div>
  );
};
