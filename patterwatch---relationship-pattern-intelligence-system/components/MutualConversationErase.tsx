  ...
  return (
    <div className="bg-zinc-900/50 border border-zinc-800 rounded-lg p-8">
      <div className="mb-8 border-b border-zinc-800 pb-6 flex justify-between items-end">
        <div>
          <h2 className="text-3xl font-bold text-red-500 tracking-tighter uppercase">
            Mutual Conversation Erase (MCE)
          </h2>
          <p className="text-zinc-500 text-xs mt-2 font-mono uppercase tracking-widest">
            Permanent Cryptographic Removal • No Recovery Possible
          </p>
        </div>
      ...
      <div className="space-y-4">
        {conversations.length > 0 ? (
          conversations.map((convo) => (
            <div
              key={convo.id}
              className="bg-zinc-950 border border-zinc-800 p-6 rounded-lg flex flex-col md:flex-row md:items-center justify-between gap-4 group hover:border-red-900/50 transition-all"
            >
          ...
          <div className="py-20 text-center border-2 border-dashed border-zinc-800 rounded-lg">
            <p className="text-zinc-600 font-mono uppercase tracking-[0.5em]">
      ...
      {showPinDialog && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/95 backdrop-blur-md p-4">
          <div className="w-full max-w-sm bg-zinc-900 border border-red-900/50 rounded-lg p-8 shadow-2xl">
            <h3 className="text-xl font-bold text-white uppercase tracking-widest text-center mb-6">
              Authorize Deletion
            </h3>
            <p className="text-zinc-500 text-xs text-center mb-6 leading-relaxed">
              This action requires your Security PIN and will permanently erase
              all data from both inboxes. <br />
