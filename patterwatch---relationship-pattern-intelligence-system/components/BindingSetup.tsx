...
import { User, Partner } from "../types/types";
...
const BindingSetup: React.FC<BindingSetupProps> = ({ user, onComplete }) => {
  ...
  const [partnerStatus, setPartnerStatus] = useState<"PENDING" | "LINKED">(
    "PENDING",
  );
    ...
    <div className="max-w-2xl mx-auto bg-zinc-900/80 border border-zinc-800 rounded-lg p-10 shadow-2xl backdrop-blur-md">
      <div className="flex justify-between mb-10">
        {[1, 2, 3].map((s) => (
          ...
          >
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center font-bold border-2 z-10 ${step >= s ? "bg-green-600 border-green-500 text-white" : "bg-zinc-800 border-zinc-700 text-zinc-500"}`}
            >
              {s}
            </div>
            <span
              className={`text-[10px] uppercase font-bold tracking-tighter ${step >= s ? "text-white" : "text-zinc-600"}`}
            >
              {s === 1 ? "PIN Set" : s === 2 ? "Partner Bind" : "Social Link"}
            </span>
            {s < 3 && (
              <div
                className={`absolute top-5 left-1/2 w-full h-[2px] -z-0 ${step > s ? "bg-green-600" : "bg-zinc-800"}`}
              ></div>
            )}
          </div>
        ))}
      ...
      <div className="min-h-[300px] flex flex-col justify-center">
        {step === 1 && (
          <div className="space-y-6 text-center">
            ...
            <p className="text-zinc-400 text-sm max-w-sm mx-auto">
              Set a 6-digit Security PIN for critical actions like MCE and
              Pattern Overrides.
            </p>
            <input
              type="password"
            ...
            <div className="bg-zinc-950 border border-zinc-800 p-6 rounded-lg inline-block w-full">
              <div className="flex items-center justify-center gap-4 mb-4">
                ...
                <div className="text-right flex-1">
                  ...
                  <p
                    className={`${partnerStatus === "LINKED" ? "text-green-500" : "text-yellow-500"} font-bold`}
                  >
                    {partnerStatus}
                  </p>
                </div>
        ...
        {step === 3 && (
          <div className="space-y-6 text-center">
            <h2 className="text-2xl font-bold text-white tracking-widest uppercase">
              Social Metadata Connection
            </h2>
            <div className="grid grid-cols-2 gap-4">
              {["WhatsApp", "Telegram", "Instagram", "Messenger"].map((app) => (
                <div
                  key={app}
                  className="bg-zinc-950 border border-zinc-800 p-4 rounded-lg flex items-center justify-between group hover:border-zinc-600 cursor-pointer"
                >
            ...
            <p className="text-[10px] text-zinc-600 font-mono uppercase">
              Activity Metadata: ✅ | Structure: ✅ | Content: ❌
            </p>
          </div>
        )}
      </div>
